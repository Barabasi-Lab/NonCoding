...
for(i in 1:length(PPIs)){
  ...
  GDA_tmp = GDA %>%
    ...
    # filter(DiseaseName %in% c("asthma", "bipolar disorder", "schizophrenia")) %>% 
    mutate(n = n()) %>%
    filter(n > 5) 
  ...
  message("\tThreads for the Sab: ", Threads)
  
  message("\tCalculating Sab.")
  sab_tmp = GDA_tmp %>% 
    select(ID = DiseaseName, Target = hgnc_symbol) %>%
    separation_Significance(G = x, 
                            N = N,
                            ST = ., 
                            correct_by_target = FALSE, 
                            Threads = Threads)
  
  sab_tmp$graph = PPIs[i]
  sab_tmp %<>% OrderNames()
  message("\tCalculating Jac.\n")
  jac_tmp = GDA_tmp %>%
  ...
  message("\tCalculating P Hypergeometric.\n")
  
  
  cl = parallel::makeCluster(96)
  ...
  Hyper_tmp = clusterApplyLB(cl, 1:length(diseases), Hyper)
  ...
  SAB_JAC_HYP[[i]] = Hyper_tmp %>% 
    bind_rows() %>% 
    dplyr::inner_join(., jac_tmp ) %>%
    dplyr::inner_join(., sab_tmp)
}

LCCs_Output %<>% bind_rows()
SAB_JAC_HYP %<>% bind_rows()

fwrite(LCCs_Output, "../data/output/LCC.csv")
fwrite(SAB_JAC_HYP, "../data/output/SAB_JAC.csv")
