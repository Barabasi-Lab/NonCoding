...
Fig_S3 = res %>%
  ...
  geom_smooth(span = 0.75, se = F, linewidth = 0.5) +
  scale_color_manual(values = values) +
  scale_x_continuous(trans = "log10") +
  scale_y_continuous(trans = "log10") +
  theme_minimal() +
...
```{r}
GDA_tmp = GDA %>% 
  select(DiseaseName, hgnc_symbol, evidence = evidences) 
...
GDA_CAT$Category_cl[GDA_CAT$Category_cl == ""] <- "other" 
